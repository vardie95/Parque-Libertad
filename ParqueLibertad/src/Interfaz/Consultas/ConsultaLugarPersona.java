...
public class ConsultaLugarPersona extends javax.swing.JFrame {
    ...
    private void ConsultaLugar(){
        String datos =PT_Ubicacion.getText();
        try {
            CallableStatement cstmt =null;
            Connection con = parquelibertad.dbConnection.conectDB();
            String opcion=CB_Ubicacion.getSelectedItem().toString();
            if(opcion.equals("Distrito")){
                cstmt =con.prepareCall("{call consultaDistrito(?,?)}");
            }else if (opcion.equals("Cantón")){
                cstmt =con.prepareCall("{call consultaCanton(?,?)}");
            }else if (opcion.equals("Provincia")){
                cstmt =con.prepareCall("{call consultaProvincia(?,?)}");
            }else{
                cstmt =con.prepareCall("{call consultaPais(?,?)}");
            }
            cstmt.registerOutParameter(2, oracle.jdbc.OracleTypes.CURSOR);
            cstmt.setString(1, datos);
            cstmt.execute();
    ...
    private void initComponents() {
        ...
        PT_Ubicacion.setBackground(new java.awt.Color(204, 204, 255));
        ...
        CB_Ubicacion.setBounds(150, 40, 99, 22);
